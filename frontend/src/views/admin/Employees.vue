<template>
  <div class="space-y-6">
    <div class="flex justify-between items-center">
      ...
      <button @click="openCreateModal" class="bg-blue-600 text-white px-4 py-2 rounded"><i class="fas fa-plus mr-1"></i> Ajouter</button>
    </div>
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow overflow-x-auto">
      <table class="w-full text-sm">
        <thead class="bg-gray-50 dark:bg-gray-700">
          <tr>
            <th class="p-3 text-left dark:text-white">Nom</th><th class="p-3 text-left dark:text-white">Email</th><th class="p-3 text-left dark:text-white">Matricule</th><th class="p-3 text-left dark:text-white">Poste</th><th class="p-3 text-left dark:text-white">Service</th><th class="p-3 text-left dark:text-white">Salaire</th><th class="p-3 text-left dark:text-white">Actions</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="emp in employees" :key="emp.id" class="border-b dark:border-gray-700">
            <td class="p-3 dark:text-white">{{ emp.name }}</td><td class="p-3 dark:text-white">{{ emp.email }}</td><td class="p-3 dark:text-white">{{ emp.matricule || '-' }}</td><td class="p-3 dark:text-white">{{ emp.position || '-' }}</td><td class="p-3 dark:text-white">{{ emp.department || '-' }}</td><td class="p-3 dark:text-white">{{ emp.base_salary || '-' }}</td>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-xl p-6 w-full max-w-md">
        ...
        <form @submit.prevent="saveEmployee">
          ...
          <div class="grid grid-cols-2 gap-4 mb-3">
            <div><label class="block text-sm dark:text-white">Matricule</label><input v-model="form.matricule" class="w-full border p-2 rounded dark:bg-gray-700 dark:text-white" /></div>
            <div><label class="block text-sm dark:text-white">Service</label><input v-model="form.department" class="w-full border p-2 rounded dark:bg-gray-700 dark:text-white" /></div>
          </div>
          <div class="mb-3"><label class="block text-sm dark:text-white">Poste</label><input v-model="form.position" class="w-full border p-2 rounded dark:bg-gray-700 dark:text-white" /></div>
          <div class="mb-3">
          ...
          <div class="mb-3" v-if="!editingEmployee">
            ...
            <PasswordInput v-model="form.password" required :error="fieldErrors.password?.[0]" />
          </div>
...
async function saveEmployee() {
  ...
  try {
    ...
    if (e.response?.status === 422 && e.response.data?.fieldErrors) {
      ...
      Swal.fire('Erreur', 'Merci de corriger les champs indiqués en rouge.', 'error')
    } else {
      Swal.fire('Erreur', e.response?.data?.message || 'Erreur inconnue', 'error')
    }
  } finally {
...
async function changePassword(emp) {
  const { value: password } = await Swal.fire({
    title: 'Nouveau mot de passe', input: 'password', inputPlaceholder: 'Mot de passe', showCancelButton: true,
