    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow overflow-x-auto">
      <table class="w-full text-sm">
        <thead class="bg-gray-50 dark:bg-gray-700">
          <tr>
            <th class="p-3 text-left dark:text-white">Titre</th>
        ...
        <tbody>
          <tr v-for="doc in documents" :key="doc.id" class="border-b dark:border-gray-700">
            <td class="p-3 dark:text-white">{{ doc.title }}</td>
            <td class="p-3 dark:text-white">{{ doc.type }}</td>
      ...
      <p v-if="documents.length === 0" class="p-4 text-gray-500 dark:text-gray-400 text-center">Aucun document.</p>
    </div>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-xl p-6 w-full max-w-md">
        ...
        <form @submit.prevent="upload" class="space-y-4">
          <div>
            ...
            <input v-model="form.title" required :class="['w-full border p-2 rounded dark:bg-gray-700 dark:text-white', fieldErrors.title ? 'border-red-500' : '']" />
          ...
          <div>
            ...
            <input type="file" @change="handleFile" required :class="['w-full border p-2 rounded dark:bg-gray-700 dark:text-white', fieldErrors.file ? 'border-red-500' : '']" />
