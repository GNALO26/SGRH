      ...
      <form @submit.prevent="saveSettings" class="space-y-4">
        <div class="grid grid-cols-2 gap-4">
          <div>
            ...
            <input type="time" v-model="form.official_opening_time" required :class="['border rounded p-2 w-full dark:bg-gray-700 dark:text-white', fieldErrors.official_opening_time ? 'border-red-500 ring-1 ring-red-500' : '']" />
          ...
          <div>
            ...
            <input type="time" v-model="form.official_closing_time" required :class="['border rounded p-2 w-full dark:bg-gray-700 dark:text-white', fieldErrors.official_closing_time ? 'border-red-500 ring-1 ring-red-500' : '']" />
        ...
        <div>
          <div class="flex items-center justify-between mb-1">
            ...
            <button type="button" @click="useMyPosition" :disabled="locating" class="text-xs bg-blue-100 text-blue-700 px-3 py-1 rounded hover:bg-blue-200 disabled:opacity-50">
        ...
        <div class="grid grid-cols-2 gap-4">
          <div>
            ...
            <input type="number" step="any" v-model="form.company_latitude" required :class="['border rounded p-2 w-full dark:bg-gray-700 dark:text-white', fieldErrors.company_latitude ? 'border-red-500 ring-1 ring-red-500' : '']" />
          ...
          <div>
            ...
            <input type="number" step="any" v-model="form.company_longitude" required :class="['border rounded p-2 w-full dark:bg-gray-700 dark:text-white', fieldErrors.company_longitude ? 'border-red-500 ring-1 ring-red-500' : '']" />
        ...
        <div>
          ...
          <input type="number" v-model="form.geofence_radius_meters" required min="1" :class="['border rounded p-2 w-full dark:bg-gray-700 dark:text-white', fieldErrors.geofence_radius_meters ? 'border-red-500 ring-1 ring-red-500' : '']" />
...
const form = ref({ official_opening_time: '08:00', official_closing_time: '20:00', company_latitude: null, company_longitude: null, geofence_radius_meters: 50 })
...
onMounted(async () => {
  try {
    ...
    if (data) {
      form.value = { ...form.value, ...data, official_opening_time: data.official_opening_time?.substring(0, 5) || '08:00', official_closing_time: data.official_closing_time?.substring(0, 5) || '20:00' }
    }
...
function useMyPosition() {
  if (!navigator.geolocation) { Swal.fire('Erreur', 'Géolocalisation non supportée.', 'error'); return }
  locating.value = true
  navigator.geolocation.getCurrentPosition(
    (pos) => { setPosition(pos.coords.latitude, pos.coords.longitude); locating.value = false; Swal.fire('Succès', 'Position actuelle définie.', 'success') },
    (err) => { locating.value = false; Swal.fire('Erreur', 'Impossible d\'obtenir votre position : ' + err.message, 'error') },
