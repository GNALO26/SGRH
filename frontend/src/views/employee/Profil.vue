<template>
  <div class="space-y-6">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow p-6 max-w-lg">
      <div v-if="!auth.user" class="text-center py-4"><i class="fas fa-spinner fa-spin text-2xl text-blue-600"></i><p class="mt-2 text-gray-500">Chargement...</p></div>
      <template v-else>
        <div class="flex items-center gap-4 mb-6">
          <img :src="avatar" class="h-16 w-16 rounded-full object-cover border-2 border-blue-200" />
          <div><h2 class="text-xl font-semibold dark:text-white">{{ user.name }}</h2><p class="text-gray-500 dark:text-gray-400">{{ user.email }}</p></div>
        </div>
        <div class="mb-6">
          ...
          <button @click="uploadAvatar" :disabled="uploading" class="mt-2 bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 disabled:opacity-50">{{ uploading ? 'Envoi...' : 'Mettre à jour' }}</button>
        </div>
        <div class="space-y-3">
...
const avatarFile = ref(null), uploading = ref(false), fieldErrors = ref({})

const avatar = computed(() => auth.user?.avatar_url || 'https://ui-avatars.com/api/?name=' + encodeURIComponent(user.value?.name || '') + '&background=0D47A1&color=fff')

function handleFile(e) { avatarFile.value = e.target.files[0] }

async function uploadAvatar() {
  if (!avatarFile.value) { Swal.fire('Erreur', 'Veuillez sélectionner un fichier.', 'warning'); return }
  uploading.value = true; fieldErrors.value = {}
  try {
    const form = new FormData(); form.append('avatar', avatarFile.value)
    const { data } = await api.post('/user/avatar', form)
    auth.user.avatar_url = data.avatar_url
    Swal.fire('Succès', 'Photo mise à jour.', 'success'); avatarFile.value = null
  } catch (e) {
