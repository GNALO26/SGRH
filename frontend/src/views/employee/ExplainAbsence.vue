<template>
  <div class="min-h-screen bg-gray-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-xl shadow-lg max-w-lg w-full p-6">
      ...
      <div v-if="absences.length === 0" class="text-center text-green-600">
        <i class="fas fa-check-circle text-3xl"></i>
        <p class="mt-2">Toutes vos absences ont été expliquées. Redirection...</p>
      </div>

      <div v-for="absence in absences" :key="absence.id" class="border rounded-lg p-4 mb-4">
        <p class="font-medium">
          Absence du <strong>{{ absence.from_date }}</strong> au <strong>{{ absence.to_date }}</strong>
        </p>
        <div class="mt-3">
          <label class="block text-sm mb-1">Explication *</label>
          <textarea
        ...
        <div class="mt-2">
          <label class="block text-sm mb-1">Justificatif (optionnel, PDF/JPG/PNG)</label>
          <input
            type="file"
        ...
        <p v-if="success[absence.id]" class="text-green-600 text-sm mt-1">Explication envoyée.</p>
      </div>
    </div>
  </div>
</template>
