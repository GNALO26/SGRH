<template>
  <aside class="w-60 bg-sidebar text-white flex flex-col h-full" :class="$attrs.class">
    <!-- Logo -->
    <div class="p-4 border-b border-gray-700 flex-shrink-0">
      <img src="/logo-sgrh.png" alt="SGRH" class="h-10 mx-auto" />
      <p class="text-center text-sm mt-2 font-semibold text-gray-200">Administrateur</p>
    </div>
    ...
    <nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
      ...
      <!-- Gestion des absences (sous-menu) -->
      <div class="space-y-1">
        <button
          @click="absOpen = !absOpen"
          class="flex items-center w-full px-3 py-2 rounded-lg transition-colors duration-200 text-gray-300 hover:bg-gray-800 hover:text-white"
        >
          <i class="fas fa-calendar-times w-5 text-center"></i>
          <span class="flex-1 ml-3 text-sm">Gestion des absences</span>
          <i :class="absOpen ? 'fas fa-chevron-down' : 'fas fa-chevron-right'" class="text-xs transition-transform duration-200"></i>
        </button>
        <div
          v-if="absOpen"
